...
def game(players = [], announces = [], appel = None, ecart_auto = False, game_auto = False, number_of_rounds = 1, just_scores = False):
    party = Party()
    if players == []:
        player = "None"
        while player != "":
    ...
    for i in range(number_of_rounds):
        ...
        while party.round.state == 2:  # ecart
            if not ecart_auto:
                ...
                while not party.round.do_ecart(party.round.players[party.round.player_turn], ecart, printing=False):
                    ecart = np.random.choice(party.round.players[party.round.player_turn].cards, size=len(party.round.chien), replace=False).tolist()
        ...
        if not game_auto:
            ...
            while party.round.state == 3:  # jeu
                ...
                card = np.random.choice(player.cards)
                while not party.round.play_hand(player, card, printing=False, just_scores=just_scores):
                    card = np.random.choice(player.cards)
    ...
    print("\n\n----------------------------------------------fin de partie-------------------------------------------------\n\n")
    scores = [(player.name, player.score) for player in party.players]
    scores.sort(key=lambda x: x[1], reverse=True)
    print("\nScores finaux (ordre décroissant) :")
...
def auto_games():
    ...
    game_auto = True
    number_of_rounds = 50
    game(players, announces, appel, ecart_auto, game_auto, number_of_rounds, just_scores=True)
